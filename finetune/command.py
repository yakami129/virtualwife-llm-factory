...
llama_sft_command: str = """
   CUDA_VISIBLE_DEVICES={cuda_visible_devices} python {finetune_path}/train_bash.py \
    --stage sft \
    --do_train \
...
class BaseFinetune(ABC):
    @abstractmethod
    def parser(self, work_dir_path: str, dataset_name: str, config: Optional[dict]):
        pass


class LLamaSftFinetune(BaseFinetune):
